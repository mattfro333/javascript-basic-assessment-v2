...
// repeats strings. Example: 4->12, 2.5->7.5, 'Awesome'->'AwesomeAwesomeAwesome'
 function tripleTheFun(param){
	 if(typeof param === 'string' && isNaN(param) === true){
		 ...
		 return parseFloat(param) + parseFloat(param) + parseFloat(param);
...
// and changes the actor object to have a property called spiderRating with a value of 10.  If their name is Tobey Maguire set spiderRating to 0.
// otherwise set spiderRating to 'not a spiderman'.
function spiderTest(actor){
	for(var prop in actor){
	if(actor[prop] === "Andrew Garfield"){
		actor.spiderRating = 10;
	}else if(actor[prop] === "Tobey Maguire"){
